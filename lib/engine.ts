...
export type Signal={referenceHour:RefHour;direction:'LONG'|'SHORT';sweepTime:string;returnTime:string;mssTime:string;entryTime:string;entry:number;stop:number;target2R:number;result:'WIN'|'LOSS'|'OPEN';r:number};
...
function candidate(candles:Candle[],h:RefHour){
 ...
 // A valid sweep needs a BODY CLOSE beyond liquidity, not just a wick.
 for(let i=0;i<xs.length;i++){if(xs[i].close<ref.low){sweep=i;dir='LONG'}else if(xs[i].close>ref.high){sweep=i;dir='SHORT'}if(sweep<0)continue;
   // Any later body-close sweep replaces the candidate. We only use the final close-back.
   for(let j=i+1;j<xs.length;j++){if(dir==='LONG'&&xs[j].close<ref.low){sweep=j;i=j;continue}if(dir==='SHORT'&&xs[j].close>ref.high){sweep=j;i=j;continue}if((dir==='LONG'&&xs[j].close>=ref.low)||(dir==='SHORT'&&xs[j].close<=ref.high)){ret=j;break}}
   if(ret>=0)break;
 }
 ...
 // Structure is rebuilt from the return candle. Ignore structure that existed before re-entry.
 let swing=-1,mss=-1;
 ...
 for(let i=swing+1;i<xs.length;i++){if(dir==='LONG'&&xs[i].close>xs[swing].high){mss=i;break}if(dir==='SHORT'&&xs[i].close<xs[swing].low){mss=i;break}}
 if(mss<0)return{ref,stage:'RETURN' as const,dir,detail:'New swing formed; waiting for body-close MSS'};
 // First valid 3-candle FVG after MSS, and the gap itself must sit inside original liquidity range.
 let fvg=-1,lo=0,hi=0;
 for(let i=mss+2;i<xs.length;i++){const a=xs[i-2],d=xs[i];if(dir==='LONG'&&d.low>a.high){lo=a.high;hi=d.low}else if(dir==='SHORT'&&d.high<a.low){lo=d.high;hi=a.low}else continue;if(lo>=ref.low&&hi<=ref.high){fvg=i;break}}
 ...
 return{ref,stage:'SIGNAL' as const,dir,detail:'Valid first-touch FVG entry',xs,sweep,ret,mss,fvg,touch,lo,hi};
}
...
export function backtest(c:Candle[],h:RefHour):Signal[]{const x=candidate(c,h);if(x.stage!=='SIGNAL'||!x.dir||!('xs'in x))return[];const{xs,sweep,ret,mss,fvg,touch}=x;const three=xs.slice(fvg-2,fvg+1);const entry=x.dir==='LONG'?x.hi:x.lo;const stop=x.dir==='LONG'?Math.min(...three.map(z=>z.low)):Math.max(...three.map(z=>z.high));const risk=Math.abs(entry-stop);if(!risk)return[];const target2R=x.dir==='LONG'?entry+2*risk:entry-2*risk;let result:'WIN'|'LOSS'|'OPEN'='OPEN',r=0;for(const z of xs.slice(touch)){const sl=x.dir==='LONG'?z.low<=stop:z.high>=stop,tp=x.dir==='LONG'?z.high>=target2R:z.low<=target2R;if(sl&&tp){result='LOSS';r=-1;break}if(sl){result='LOSS';r=-1;break}if(tp){result='WIN';r=2;break}}return[{referenceHour:h,direction:x.dir,sweepTime:xs[sweep].time,returnTime:xs[ret].time,mssTime:xs[mss].time,entryTime:xs[touch].time,entry,stop,target2R,result,r}]}
