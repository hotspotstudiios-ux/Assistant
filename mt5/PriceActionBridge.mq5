...
#property description "Resilient chunked M1 bridge for Price Action Lab"
...
int SendChunk(string sym,MqlRates &rates[],int oldestPos,int newestPos,int chunkIndex,int chunkTotal,int offset,string uploadId,int retryCount){
 ...
 Print("PriceActionBridge chunk ",chunkIndex+1,"/",chunkTotal," try ",retryCount+1," HTTP ",status,": ",CharArrayToString(result));
